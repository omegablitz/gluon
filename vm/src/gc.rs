...
pub struct Gc {
    ...
    #[cfg_attr(feature = "serde_derive", serde(skip))]
    watchers: Option<Box<Watcher>>,
    #[cfg_attr(feature = "serde_derive", serde(skip))]
    protected: Option<crate::base::serialization::NodeMap>,
...
impl Gc {
    ...
    pub fn add_protected(&mut self, de_seed: &crate::serialization::UnsafeDeSeed) {
        self.protected = Some(de_seed.gc_map.clone());
    }

    pub fn new_child_gc(&self) -> Gc {
        let mut new_gc = Gc::new(self.generation.next(), self.memory_limit);
        new_gc.watchers = self.watchers.clone();
        new_gc.protected = self.protected.clone();
        new_gc
    }
    ...
    pub unsafe fn sweep(&mut self) {
        ...
        {
            let mut collect = |maybe_protect_map: Option<&HashMap<*const (), crate::base::serialization::Id>>| {
                ...
                loop {
                    ...
                    match *maybe_header {
                        Some(ref mut header) => {
                            ...
                            let is_protected = match maybe_protect_map {
                                Some(protect_map) => protect_map.contains_key(&(header.value() as *const ())),
                                None => false,
                            };

                            if !header.marked.get() && !is_protected {
        ...
        let mut maybe_watcher = &mut self.watchers;
        loop {
            let mut free = false;
            ...
            match *maybe_watcher {
                ...
                Some(ref mut watcher) => {
                    let maybe_arc = watcher.elem.upgrade();
                    match maybe_arc {
                        None => {
                            free = true;
                            maybe_next = watcher.next.take();
                        },
                        Some(arc) => {
            ...
            if free {
                *maybe_watcher = maybe_next;
            } else {
                maybe_watcher = &mut maybe_watcher.as_mut().unwrap().next;
            }
        }
